...
class CFRuntime
{
	 ...
	 */
	 	...
	 	public static function getAuthenticatedURL($bucket,$key,$secretk, $uri, $lifetime, $hostBucket = false, $https = false)
	{
		$expires = self::__getTime() + $lifetime;
		$uri = str_replace(array('%2F', '%2B'), array('/', '+'), rawurlencode($uri));
		return sprintf(($https ? 'https' : 'http').'://%s/%s?AWSAccessKeyId=%s&Expires=%u&Signature=%s',
		// $hostBucket ? $bucket : $bucket.'.s3.amazonaws.com', $uri, self::$__accessKey, $expires,
		$hostBucket ? $bucket : self::$endpoint.'/'.$bucket, $uri,$key, $expires,
		...
		public static function __getTime()
	{
		...
		private static function __getHash($string,$secretk)
	{
		return base64_encode(extension_loaded('hash') ?
		hash_hmac('sha1', $string, $secretk, true) : pack('H*', sha1(
		(str_pad(self::$__secretKey, 64, chr(0x00)) ^ (str_repeat(chr(0x5c), 64))) .
		pack('H*', sha1((str_pad($secretk, 64, chr(0x00)) ^
