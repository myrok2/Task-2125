...
define('WP_MASQUERADE_USE_SESSIONS', false);
...
class WPMasquerade {
	...
	public function ajax_masq_login(){
		...
		$identifer = filter_input(INPUT_POST, 'identifier', FILTER_SANITIZE_STRING);
		$reset     = filter_input(INPUT_POST, 'reset', FILTER_VALIDATE_BOOLEAN);

		if(!$reset && is_null($uid) && is_null($identifer))
			wp_die('Security Check');
		...
		if(!$reset) {
			if (is_numeric($identifer)) {
				...
				$operator = filter_var($identifer, FILTER_VALIDATE_EMAIL) ? 'email' : 'login';
				$user = get_user_by($operator, $identifer);
				if ($user) {
					$uid = $user->ID;
				} else {
