document.addEventListener('DOMContentLoaded', function () {
	...
	var active_requests = [],
		...
		search_boxes = document.querySelectorAll('input[name=s]:not([data-dwls-ignore])');
	...
	/**
	 * Get an element's closest anscestor by a selector
	 *
	 * @param elem
	 * @param CSS style selector
	 * @returns elem|null
	 */
	function find_parent_by_selector(elm, selector) {
	...
	/**
	 * Toggle loading indicator on element
	 *
	 * @param elem
	 * @param bool
	 * @returns null
	 */
	function set_loading_state(elm, setValue) {
		if (!elm) return;
		var operator = setValue ? 'add' : 'remove';
		elm.parentNode.classList[operator]('dwls--loading');
	}
	...
	function do_search(e) {
		...
		if ('' !== e.target.value) {
			...
			set_loading_state(e.target, true);
			...
			xmlhttp.onreadystatechange = function () {
				if (xmlhttp.readyState == 4) {
					if (xmlhttp.status == 200) {
						...
						if (false /* Pass option for selector */) {
							...
							var parentSelector = e.target.dataset.dwlsAppend;

							var appendNode = find_parent_by_selector(e.target, parentSelector);

							if (appendNode) {
								appendNode.appendChild(results_element);
							} else {
								e.target.parentNode.appendChild(results_element)
							}
					...
					set_loading_state(e.target, false);
				}
			};
	...
	function hide_search() {
		...
		if (document.getElementById('dwls-results')) {
			var node = document.getElementById('dwls-results');
