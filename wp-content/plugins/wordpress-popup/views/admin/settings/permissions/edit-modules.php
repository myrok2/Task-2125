	...
	<div class="sui-box-settings-col-2">
		...
		if ( 0 === count( $modules ) && ! $filtered ) { ?>
			...
			$this->render(
				...
				[
					'content' => array(
						esc_html__( "You haven't created any module yet.", 'wordpress-popup' )
					),
				]
			);
			?>
			...
			<?php if ( 0 === count( $modules ) && $filtered ) {
				...
				$this->render(
					...
					[
						'type'    => 'info',
						'content' => array(
							esc_html__( "You don't have any module corresponding to these filter parameters.", 'wordpress-popup' )
						),
					]
				);
