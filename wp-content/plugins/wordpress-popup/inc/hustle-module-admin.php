 ...
 */
		...
		*/
		...
		public function show_sendgrid_update_notice() {
			...
			<div id="hustle-sendgrid-update-notice" class="notice notice-warning is-dismissible" data-name="hustle_sendgrid_update_showed" data-nonce="<?php echo esc_attr( wp_create_nonce( 'hustle_dismiss_notification' ) ); ?>">
				...
				<p><a href="<?php echo esc_url( $integrations_url ); ?>" class="button-primary"><?php esc_html_e( 'Review Integrations', 'wordpress-popup' ); ?></a></p>
			</div>
			<?php
		}
	 ...
	 */
		...
		public function register_scripts( $page_slug ) {
			...
			$optin_vars = array(
				...
				'is_new' => self::is_new(), // TODO: use admin_utils instead.
				'current' => array(),
				...
				'messages' => array(
					...
					'cant_empty' => __( 'This field can\'t be empty.', 'wordpress-popup' ),
					'url_required_error_message' => __( 'Your website url is required.', 'wordpress-popup' ),
					'gdpr_required_error_message' => __( 'Please accept the terms and try again.', 'wordpress-popup' ),
					'validation_message' => __( 'Please enter a valid {field}.', 'wordpress-popup' ),
					'date_validation_message' => __( 'Please enter a valid date.', 'wordpress-popup' ),
					'time_validation_message' => __( 'Please enter a valid time.', 'wordpress-popup' ),
					'recaptcha_validation_message' => __( 'reCAPTCHA verification failed. Please try again.', 'wordpress-popup' ),
					...
					'commons' => array(
						...
						'generic_ajax_error' => __( 'Something went wrong with the request. Please reload the page and try again.', 'wordpress-popup' ),
						'module_imported' => __( 'Module successfully imported.', 'wordpress-popup' ),
						'module_duplicated' => __( 'Module successfully duplicated.', 'wordpress-popup' ),
						'module_tracking_reset' => __( "Module's tracking data successfully reset.", 'wordpress-popup' ),
						'module_deleted' => __( 'Module successfully deleted.', 'wordpress-popup' ),
						'shortcode_copied' => __( 'Shortcode copied successfully.', 'wordpress-popup' ),
					),
				),
			);
	 ...
	 */
		public function add_plugin_action_links( $actions, $plugin_file, $plugin_data, $context ) {
			...
			if ( $plugin === $plugin_file ) {
				...
				$links = [
					'settings' => '<a href="'. $settings_url .'">' . esc_html__( 'Settings', 'wordpress-popup' ) . '</a>',
					'docs' => '<a href="https://premium.wpmudev.org/project/hustle/#wpmud-hg-project-documentation?utm_source=hustle&utm_medium=plugin&utm_campaign=hustle_pluginlist_docs" target="_blank">' . esc_html__( 'Docs', 'wordpress-popup' ) . '</a>',
				];
				...
				if ( Opt_In_Utils::_is_free() ) {
					...
					if ( is_network_admin() || ! is_multisite() ) {
						$links['upgrade'] = '<a href="' . esc_url( $url ) . '" aria-label="' . esc_attr( __( 'Upgrade to Hustle Pro', 'wordpress-popup' ) ) . '" target="_blank" style="color: #8D00B1;">' . esc_html__( 'Upgrade', 'wordpress-popup' ) . '</a>';
					}
				} else {
					if ( ! lib3()->is_member() ) {
						$links['renew'] = '<a href="https://premium.wpmudev.org/?utm_source=hustle&utm_medium=plugin&utm_campaign=hustle_pluginlist_renew" target="_blank" style="color: #8D00B1;">' . esc_html__( 'Renew Membership', 'wordpress-popup' ) . '</a>';
					}
				}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			if ( Opt_In::$plugin_base_file === $plugin_file ) {
				if ( Opt_In_Utils::_is_free() ) {
					$row_meta = array(
						'rate' => '<a href="https://wordpress.org/support/plugin/wordpress-popup/reviews/#new-post" target="_blank">' . esc_html__( 'Rate Hustle', 'wordpress-popup' ) . '</a>',
						'support' => '<a href="https://wordpress.org/support/plugin/wordpress-popup/" target="_blank">' . esc_html__( 'Support', 'wordpress-popup' ) . '</a>',
					);
				} else {
					$row_meta = array(
						'support' => '<a href="https://premium.wpmudev.org/hub/support/#wpmud-chat-pre-survey-modal" target="_blank">' . esc_html__( 'Premium Support', 'wordpress-popup' ) . '</a>',
					);
				}

				$row_meta['roadmap'] = '<a href="https://premium.wpmudev.org/roadmap/" target="_blank">' . esc_html__( 'Roadmap', 'wordpress-popup' ) . '</a>';
	 ...
	 */
		...
		public function get_provider_migration_notice_html( $provider ){
			$current_user = wp_get_current_user();
			...
			?>
			<div id='<?php echo esc_attr( "hustle_migration_notice__$provider" ); ?>' class="hustle-notice notice notice-warning hustle-provider-notice" data-name="<?php echo esc_attr( $provider ); ?>" style="display: none">
				<p>
				<?php $this->get_provider_migration_content( $provider, $username ); ?>
				</p>
				<p><a href="<?php echo esc_url( $migrate_url ); ?>" class="button-primary"><?php esc_html_e( 'Migrate Data', 'wordpress-popup' ); ?></a><a style="margin-left:20px; text-decoration: none;" href="#" class="dismiss-provider-migration-notice" data-name="<?php echo esc_attr( $provider ); ?>"><?php esc_html_e( 'Remind me later', 'wordpress-popup' ); ?></a></p>
			</div>
			<?php
		}

		public function get_provider_migration_content( $provider, $username = '' ) {
			switch ( $provider ) {
				case 'constantcontact':
					$msg = sprintf( __( "Hey %s, we have updated our Constant Contact integration to support the latest v3.0 API." ), $username );
					break;
				case 'infusionsoft':
					$msg = sprintf( __( "Hey %s, we have updated our InfusionSoft integration to support the latest REST API." ), $username );
					break;
			...
			$msg .=  __( " Since you are connected to the old API version, we recommend you to migrate your integration to the latest API version as we'll cease to support the deprecated API at some point.", 'wordpress-popup' );
