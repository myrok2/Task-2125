	...
	public function get_module_meta_names( $module_type = '', $module_mode = '', $with_display_name = false ) {
		...
		if ( ! $with_display_name ) {
			...
			$metas[0] = [
				...
				'label' => self::SOCIAL_SHARING_MODULE !== $module_type ? __( 'Content', 'wordpress-popup' ) : __( 'Services', 'wordpress-popup' ),
			];

			$metas[3] = [
				'name'  => self::KEY_DESIGN,
				'label' => __( 'Appearance', 'wordpress-popup' ),
			];

			$metas[5] = [
				'name'  => self::KEY_VISIBILITY,
				'label' => __( 'Visibility', 'wordpress-popup' ),
			];
			...
			if ( 'optin' === $module_mode ) {
				...
				$metas[2] = [
					...
					'label' => __( 'Integrations', 'wordpress-popup' ),
				];
			}
			...
			if ( self::SOCIAL_SHARING_MODULE !== $module_type ) {
				...
				$metas[6] = [
					...
					'label' => __( 'Behavior', 'wordpress-popup' ),
				];
			}
			...
			if ( self::SOCIAL_SHARING_MODULE === $module_type || self::EMBEDDED_MODULE === $module_type ) {
				...
				$metas[4] = [
					...
					'label' => __( 'Display Options', 'wordpress-popup' ),
				];
			}
