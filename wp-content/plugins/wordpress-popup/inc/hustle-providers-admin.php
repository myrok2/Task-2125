		...
		if ( 'external-redirect' === $action && true === $migration ) {
			...
			if ( $nonce && wp_verify_nonce( $nonce, 'hustle_provider_external_redirect' ) ) {
				...
				if( 'constantcontact' === $slug ){
					$response['migration_notificaiton']['message'] = sprintf( esc_html__( "%s integration successfully migrated to the v3.0 API version.", 'wordpress-popup' ), '<strong>' . esc_html__( 'Constant Contact', 'wordpress-popup' ) . '</strong>' );
				}

				if( 'infusionsoft' === $slug ){
					$response['migration_notificaiton']['message'] = sprintf( esc_html__( "%s integration successfully migrated to use the REST API.", 'wordpress-popup' ), '<strong>' . esc_html__( 'InfusionSoft', 'wordpress-popup' ) . '</strong>' );
				}
