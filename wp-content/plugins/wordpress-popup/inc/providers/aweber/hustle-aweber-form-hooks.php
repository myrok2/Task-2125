...
class Hustle_Aweber_Form_Hooks extends Hustle_Provider_Form_Hooks_Abstract {
	...
	public function add_entry_fields( $submitted_data ) {
		...
		try {
			...
			$error_message = __( 'Something went wrong. Unable to add subscriber. ', 'wordpress-popup' );
			if ( empty( $subscriber ) ) {
			...
			if ( 'created' === $action ) {
				...
				if ( ! empty( $subscriber_custom_fields ) && ! empty( $subscribe_data['custom_fields'] ) ) {
					...
					if ( ! empty( $found_missing_field ) ) {
						$details = __( 'Some fields are not successfully added: ', 'wordpress-popup' ) . implode( ', ', $found_missing_field );
