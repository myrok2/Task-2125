...
class Hustle_Local_List_Form_Settings extends Hustle_Provider_Form_Settings_Abstract {
	...
	public function first_step_callback( $submitted_data ) {
		...
		$step_html = Hustle_Provider_Utils::get_integration_modal_title_markup(
			__( 'Hustle\'s Local List', 'wordpress-popup' ),
			sprintf( __( 'Save the submissions in your database so you can access them or export them from the %1$sEmail Lists%2$s page. Local list (when active) also stores the status of active third party apps for each submission.', 'wordpress-popup' ),
			...
			'list_id_setup' => array(
				...
				'elements' => array(
					'label' => array(
						'type'  => 'label',
						'for'   => 'local_list_name',
						'value' => __( 'Email List name', 'wordpress-popup' ),
					),
					'local_list_name' => array(
						...
						'value' => $name,
						'id'    => 'local_list_name',
						'placeholder' => __( 'Choose a name for this email list', 'wordpress-popup' ),
					),
					'description' => array(
						'type'  => 'description',
