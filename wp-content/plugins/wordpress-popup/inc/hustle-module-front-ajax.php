			...
			try {
				...
				if ( ! $token ) {
					throw new Exception( ! empty( $validation_message ) ? $validation_message
							: $validation_messageesc_html__( 'reCAPTCHA must be verified to submit the form.', 'wordpress-popup' ) );
				}
				...
				if ( empty( $response_body ) || ! $response_body->success ) {
					throw new Exception( !empty( $validation_message ) ? $validation_message
							: esc_html__( 'reCAPTCHA validation failed. Please try again.', 'wordpress-popup' ) );
				}
				...
				if ( 'v3_recaptcha' === $recaptcha_version ) {
					...
					if ( $selected_score > $response_body->score ) {
						throw new Exception( esc_html__( 'reCAPTCHA validation failed. The score is too low.', 'wordpress-popup' ) );
					}
				}
